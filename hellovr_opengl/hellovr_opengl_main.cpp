...
void CMainApplication::printStatusData()
{
	for (vr::TrackedDeviceIndex_t unDevice = 0; unDevice < vr::k_unMaxTrackedDeviceCount; unDevice++)
	{
		...
		if (m_pHMD->GetControllerState(unDevice, &state, sizeof(state)))
		{
			...
			vr::VRControllerState_t controllerState;
			vr::ETrackedDeviceClass trackedDeviceClass = vr::VRSystem()->GetTrackedDeviceClass(unDevice);*/
			float Battery = m_pHMD->GetFloatTrackedDeviceProperty(unDevice, vr::ETrackedDeviceProperty::Prop_DeviceBatteryPercentage_Float,peError);
	...
	vr::ETrackedDeviceProperty::Prop_DeviceBatteryPercentage_Float:
		vr::
	case vr::ETrackedDeviceClass::TrackedDeviceClass_GenericTracker:
		vr::VRSystem()->GetDeviceBatteryPercentage_float
		// print positiona data for a general vive tracker.
		break;
		*/
}
